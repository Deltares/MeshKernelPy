...
def test_mesh2d_orthogonalization_inconsistency():
    """Tests `mesh2d_refine_based_on_gridded_samples` with non contiguos arrays"""
    ...
    mesh_refinement_parameters = MeshRefinementParameters(min_edge_size=min_edge_size,  # always in meters
                                                          ...
                                                          connect_hanging_nodes=True,
                                                          # set to False to do multiple refinement steps (e.g. for multiple regions)
                                                          smoothing_iterations=2,
                                                          max_courant_time=120)
    ...
    ortho_values = mk.mesh2d_get_orthogonality().values
    print(f"num orthogonality values {len(ortho_values)}")  # TODO: couple back to uds, currently ordering mismatch: https://github.com/Deltares/MeshKernelPy/issues/72


    print("max ortho after refining",ortho_values.max())  # TODO: couple back to uds, currently ordering mismatch: https://github.com/Deltares/MeshKernelPy/issues/72
    ...
    delete_pol_geom = GeometryList(x_coordinates=xx,
                                   y_coordinates=yy)  # TODO: .copy()/to_numpy() makes the array contiguous in memory, which is necessary for meshkernel.mesh2d_delete()
